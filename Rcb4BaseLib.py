...
class Rcb4BaseLib:
    ...
    def moveComToDeviceCmd(icsNum, offset, destData):
        ...
        if ((icsNum < 0) or (Rcb4BaseLib.IcsDeviceSize < icsNum) or (offset < 0) or (
                Rcb4BaseLib.IcsDeviceDataSize < (offset + destSize))):
            return -1, buf
    ...
    def moveDeviceToComCmd(icsNum, offset, dataSize):
        ...
        if ((icsNum < 0) or (Rcb4BaseLib.IcsDeviceSize < icsNum) or (offset < 0) or (
                Rcb4BaseLib.IcsDeviceDataSize < (offset + dataSize))):
            return -1, buf
    ...
    def getSinglePos(self, id, sio):
        ...
        retf, retbuf = self.moveDeviceToComCmdSynchronize(self.icsNum2id(id, sio),
                                                          Rcb4BaseLib.DeviceAddrOffset.MotorPositionAddressOffset.value,
                                                          2)
    ...
    def userParmeterAddr(parmeterNum):
        if (0 < parmeterNum <= Rcb4BaseLib.UserParmeterCount):
            return Rcb4BaseLib.RamAddr.UserParmeterRamAddress.value + (
                    (parmeterNum - 1) * Rcb4BaseLib.UserParmeterSingleDataCount)
        else:
            return -1
    ...
    def resetProgramCounter(self):
        buf = [Rcb4BaseLib.RomAddr.MainLoopCmd.value & 0xff, (Rcb4BaseLib.RomAddr.MainLoopCmd.value >> 8) & 0xff, 0, 0,
               0, 0, 0, 0, 0, 0]
